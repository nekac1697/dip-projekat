...
const App = () => {
  ...
  const { data: trenutniKorisnik } = useTrenutniKorisnik();
  ...
  return (
    <div className="flex items-center h-full justify-center">
      <div className="flex flex-col">
        <h1 className="text-3xl md:text-6xl text-white text-center">Ko gleda?</h1>
        <div className="flex items-center justify-center gap-8 mt-10">
          <div onClick={() => selectProfile()}>
            <UserCard name={trenutniKorisnik?.name} />
          </div>
        </div>
      </div>
    </div>
